...
Под ред. Г. И. Немкова — М.: Недра, 1989.
                """
                # Используем reply_markup для обычной клавиатуры
                await bot.send_message(
                    chat_id=chat_id,
                    text=response,
...
🏷️ С классификацией: {stats['with_classification']}
        ...
        # Отвечаем на callback - ИСПРАВЛЕНО для вашей версии MAX API
        try:
            # Пробуем отправить пустой callback
            await bot.send_callback(callback.callback_id)
        except Exception as e:
            try:
                # Альтернативный вариант с текстом
                await bot.send_callback(callback_id=callback.callback_id, text=" ")
            except Exception as e2:
                logger.warning(f"Не удалось отправить callback: {e2}")
